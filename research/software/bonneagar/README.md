...
# Ansible Automation for Komodo

This provides tooling for ansible-in-docker using an ansible execution environment to manage Komodo periphery servers.
...
# 2. Bootstrap (generates vault password, stores in 1Password, creates encrypted inventory)
#    You need your 1Password Connect token
bun run ansible.ts bootstrap --op-token "$(cat /etc/connect/token)"

# 3. Sync vault password to local .env (if not already done)
OP_CONNECT_TOKEN="$(cat /etc/connect/token)" bun run ansible.ts sync

# 4. Start the ansible container
docker compose up -d
...
| Command | Description |
|---------|-------------|
| `bootstrap` | Generate vault password, store in 1Password/Komodo, create encrypted inventory |
...
```bash
# Basic bootstrap (passwordless sudo)
bun run ansible.ts bootstrap --op-token "your-token"

# With sudo password (if required)
bun run ansible.ts bootstrap --op-token "your-token" --become-pass "sudo-password"

# Force regeneration of vault password
bun run ansible.ts bootstrap --op-token "your-token" --force

# Custom Komodo Core URL
bun run ansible.ts bootstrap --op-token "your-token" --periphery-url "wss://komodo.example.com"
```

### What Bootstrap Does

1. Generates a cryptographically secure 64-character vault password
2. Stores secrets in 1Password (`dev-baile/ansible-automation` item):
   - `vault_password` - the ansible vault password
   - `op_connect_token` - your 1Password Connect token
   - `become_pass` - sudo password (if provided)
3. Sets `VAULT_PASS` variable in Komodo (for Actions)
4. Encrypts secrets using ansible-vault
5. Generates `inventory/komodo.yml` with encrypted values
6. Writes vault password to `.env` for local use

### Environment Variables

| Variable | Description |
|----------|-------------|
| `OP_CONNECT_HOST` | 1Password Connect server URL (default: `http://localhost:8080`) |
| `OP_CONNECT_TOKEN` | 1Password Connect access token |
| `OP_CONNECT_TOKEN_FILE` | Path to file containing token (default: `/etc/connect/token`) |
| `OP_VAULT` | 1Password vault name (default: `dev-baile`) |
| `KOMODO_HOST` | Komodo server URL (default: `https://komodo.local`) |
...
- 1Password Connect running (for secrets management)
...
The bootstrap script creates a layered secrets management system:

```
┌─────────────────────────────────────────────────────────────────────┐
│                         1PASSWORD CONNECT                           │
│  Item: dev-baile/ansible-automation                                 │
│  Fields: vault_password, op_connect_token, become_pass              │
└─────────────────────────────────────────────────────────────────────┘
                              │
            ┌─────────────────┼─────────────────┐
            ▼                 ▼                 ▼
┌───────────────────┐ ┌───────────────┐ ┌───────────────────┐
│    LOCAL .env     │ │    KOMODO     │ │    INVENTORY      │
│                   │ │               │ │                   │
│ ANSIBLE_VAULT_    │ │ VAULT_PASS    │ │ Vault-encrypted:  │
│ PASSWORD=xxx      │ │ (secret var)  │ │ - op_connect_token│
│                   │ │               │ │ - become_pass     │
└───────────────────┘ └───────────────┘ └───────────────────┘
        │                     │                   │
        ▼                     ▼                   ▼
┌───────────────────────────────────────────────────────────────────┐
│                      ANSIBLE CONTAINER                             │
│  - Vault password injected via ANSIBLE_VAULT_PASSWORD env var      │
│  - Decrypts inventory secrets at runtime                           │
...
If you need to recover the vault password:

```bash
# Sync from 1Password to local .env
OP_CONNECT_TOKEN="$(cat /etc/connect/token)" bun run ansible.ts sync

# Or check status
OP_CONNECT_TOKEN="$(cat /etc/connect/token)" bun run ansible.ts status
```

### Rotating Secrets

To rotate the vault password and re-encrypt all secrets:

```bash
# Force regeneration
bun run ansible.ts bootstrap --op-token "$(cat /etc/connect/token)" --force
```

To update individual secrets (e.g., new OP Connect token):

```bash
bun run ansible.ts reencrypt --op-token "new-token-here"
```
