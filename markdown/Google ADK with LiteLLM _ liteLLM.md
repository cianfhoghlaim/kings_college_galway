...
tags:
  - "clippings"
---
![](https://docs.litellm.ai/assets/ideal-img/litellm_adk.186a0d5.1920.png)
...
```bash
Install dependenciespip install google-adk litellm
```
...
```python
Setup environment and API keysimport os
import asyncio
...
```python
Weather tool implementationdef get_weather(city: str) -> dict:
...
```python
Agent interaction helper functionasync def call_agent_async(query: str, runner, user_id, session_id):
...
```python
OpenAI model implementation# Create an agent powered by OpenAI's GPT model
...
```python
Anthropic model implementation# Create an agent powered by Anthropic's Claude model
...
```python
Gemini model implementation# Create an agent powered by Google's Gemini model
...
```python
LiteLLM proxy integration# Set your LiteLLM Proxy credentials as environment variables
